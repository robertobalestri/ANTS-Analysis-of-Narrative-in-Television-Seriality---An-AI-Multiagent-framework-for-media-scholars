...
export const SeriesSetupManager: React.FC<SeriesSetupManagerProps> = ({
  ...
  const [seasonInput, setSeasonInput] = useState('01');
  const [episodeInput, setEpisodeInput] = useState('01');
  ...
  const refreshSeries = async (nextSelectedSeries?: string, signal?: AbortSignal) => {
    const [libraryResponse, allSeriesResponse] = await Promise.all([
      api.getLibrarySeries({ signal }),
      api.getSeries({ signal }),
    ]);
  ...
  const refreshStatus = async (seriesCodeToLoad: string, signal?: AbortSignal) => {
    ...
    const response = await api.getLibrarySeriesStatus(seriesCodeToLoad, { signal });
  ...
  useEffect(() => {
    ...
    void refreshSeries(undefined, controller.signal);
  ...
  useEffect(() => {
    ...
    if (selectedSeries) {
      void refreshStatus(selectedSeries, controller.signal);
    } else {
      setStatus(null);
      onStatusChange(null);
    }
  ...
  const handleAddSeason = async () => {
    ...
    setIsSubmitting(true);
    const formattedSeason = formatSeasonCode(seasonInput);
    const response = await api.createLibrarySeasons(selectedSeries, [formattedSeason]);
    setIsSubmitting(false);
  ...
  const handleAddEpisode = async () => {
    ...
    setIsSubmitting(true);
    const formattedEpisode = formatEpisodeCode(episodeInput);
    const response = await api.createLibraryEpisodes(selectedSeries, assignmentSeason, [formattedEpisode]);
    setIsSubmitting(false);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      void refreshStatus(selectedSeries);
    }, 5000);
