...
export const SeriesManagerPanel: React.FC<SeriesManagerPanelProps> = ({ seriesData, onRefresh }) => {
  ...
  const handleAddSeason = async () => {
    ...
    const formattedSeason = formatSeasonCode(newSeason);
    const response = await api.createLibrarySeasons(seriesData.code, [formattedSeason]);
  ...
  const handleAddEpisode = async (season: string) => {
    ...
    const response = await api.createLibraryEpisodes(seriesData.code, season, [formattedEpisode]);
    setIsSubmitting(false);
