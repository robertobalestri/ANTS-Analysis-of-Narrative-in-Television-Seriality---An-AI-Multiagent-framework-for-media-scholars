...
export const VectorStoreTabManager: React.FC<VectorStoreTabManagerProps> = ({
  ...
  useEffect(() => {
    ...
    void fetchEntries(undefined, controller.signal);
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    void fetchClusters(undefined, controller.signal);
    return () => controller.abort();
  }, [activeTab, entries, series]);

  // Fetch entries from API
  const fetchEntries = async (query?: string, signal?: AbortSignal) => {
    try {
      const response = await request(() =>
        query
          ? api.searchVectorStore(series, query, { signal })
          : api.searchVectorStore(series, undefined, { signal })
      );
