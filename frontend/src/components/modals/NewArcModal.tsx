...
export const NewArcModal: React.FC<NewArcModalProps> = ({
  ...
  const handleSubmit = () => {
    const initialProgression: Omit<Partial<ArcProgression>, 'id'> = {
      ...
      season: formatSeasonCode(progressionSeason),
      episode: formatEpisodeCode(progressionEpisode),
  ...
  const handleGenerateContent = async () => {
    ...
    try {
      const response = await api.generateProgression(
        null,
        series,
        formatSeasonCode(progressionSeason),
        formatEpisodeCode(progressionEpisode),
        title,
        description
      );
  ...
  const handleGenerateAllProgressions = async () => {
    ...
    try {
      const seasonEpisodes = episodes
        .filter(ep => ep.season === formatSeasonCode(progressionSeason))
